...
/*
 * Parameters measured from captures/purple_ball_640x480.jpg after the same
 * 160x120 RGB565 conversion used by the on-device vision pipeline.
 */
#define PURPLE_BLUE_MIN 58
#define PURPLE_RED_MIN 30
#define PURPLE_GREEN_MIN 25
#define PURPLE_RED_MAX 170
#define PURPLE_GREEN_MAX 170
#define PURPLE_BLUE_OVER_RED 12
#define PURPLE_BLUE_OVER_GREEN 12

static bool is_purple_shell(const uint8_t *pixels, size_t index)
{
    int red;
    int green;
    int blue;
    pixel_rgb(pixels, index, &red, &green, &blue);
    return blue >= PURPLE_BLUE_MIN &&
           red >= PURPLE_RED_MIN &&
           green >= PURPLE_GREEN_MIN &&
           red <= PURPLE_RED_MAX &&
           green <= PURPLE_GREEN_MAX &&
           blue >= red + PURPLE_BLUE_OVER_RED &&
           blue >= green + PURPLE_BLUE_OVER_GREEN;
}
...
void purple_ball_vision_process(const uint8_t *rgb565, size_t width, size_t height,
                         ball_vision_result_t *result)
{
    ...
    for (size_t seed = 0; seed < pixel_count; ++seed) {
        if (s_seen[seed] || !is_purple_shell(rgb565, seed)) continue;
        size_t head = 0;
        size_t tail = 0;
        s_seen[seed] = 1;
        ...
        while (head < tail) {
            ...
            for (int dy = -1; dy <= 1; ++dy) {
                for (int dx = -1; dx <= 1; ++dx) {
                    ...
                    if (s_seen[neighbour] ||
                        !is_purple_shell(rgb565, neighbour)) continue;
                    s_seen[neighbour] = 1;
        ...
        if (area < MIN_PURPLE_COMPONENT_PIXELS || component_width < 2 ||
...
void purple_ball_vision_draw_overlay(uint8_t *rgb565, size_t width, size_t height,
                              const ball_vision_result_t *result)
{
    purple_ball_vision_draw_overlay_color(rgb565, width, height, result,
                                          0xf81f);
}

void purple_ball_vision_draw_overlay_color(uint8_t *rgb565, size_t width,
                                           size_t height,
                                           const ball_vision_result_t *result,
                                           uint16_t color)
{
