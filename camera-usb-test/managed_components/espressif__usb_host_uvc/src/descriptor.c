...
int parse_configuration(libusb_config_descriptor_t *config, const uint8_t *buffer, int size)
{
    ...
    /*
     * Some low-cost UVC cameras put the bulk endpoint descriptor before the
     * class-specific VideoStreaming descriptors. libusb normally associates
     * everything after an endpoint with endpoint->extra, while libuvc expects
     * the VS input header, formats and frames in interface->extra. Repair that
     * non-standard ordering when the endpoint extra starts with a CS_INTERFACE
     * descriptor (0x24).
     */
    for (int i = 0; i < config->bNumInterfaces; ++i) {
        libusb_interface_t *interface = &config->interface[i];
        for (int alt = 0; alt < interface->num_altsetting; ++alt) {
            libusb_interface_descriptor_t *setting = &interface->altsetting[alt];
            if (setting->bInterfaceClass != 0x0e ||
                setting->bInterfaceSubClass != 0x02) {
                continue;
            }
            for (int ep = 0; ep < setting->bNumEndpoints; ++ep) {
                libusb_endpoint_descriptor_t *endpoint = &setting->endpoint[ep];
                if (endpoint->extra_length < 2 || endpoint->extra[1] != 0x24) {
                    continue;
                }
                uint8_t *combined = realloc(setting->extra,
                                             setting->extra_length +
                                             endpoint->extra_length);
                LIBUSB_GOTO_ON_FALSE(combined);
                memcpy(combined + setting->extra_length, endpoint->extra,
                       endpoint->extra_length);
                setting->extra = combined;
                setting->extra_length += endpoint->extra_length;
                free(endpoint->extra);
                endpoint->extra = NULL;
