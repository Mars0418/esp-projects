...
{
    (void)user_pointer;
    if (!atomic_load(&s_streaming) || !frame || !frame->data) return;
    s_received_frames++;
    ...
    if (slot_index < 0 && xQueueReceive(s_frame_queue,&slot_index,0)==pdPASS) {
        /* This slot was queued, not being decoded. Retain its busy reservation
         * and replace the obsolete frame, keeping latency to one fresh frame. */
        s_dropped_frames++;
    }
    if (slot_index < 0) {
        s_dropped_frames++;
...
static void frame_display_task(void *argument)
{
    ...
    while (true) {
        digit_runtime_idle();
        if (xQueueReceive(s_frame_queue, &slot_index, pdMS_TO_TICKS(50)) != pdPASS) {
            continue;
        }
        if (!atomic_load(&s_streaming) || esp_timer_get_time()-s_slots[slot_index].captured_at_us>400000) {
            release_mjpeg_slot(slot_index);
            continue;
        }
        ...
        if (decode_error == ESP_OK && output.width == DECODED_WIDTH &&
            ...
            if (atomic_load(&s_streaming)) {
                digit_runtime_process(s_decoded_frame, s_slots[slot_index].captured_at_us);
                s_displayed_frames++;
            }
        } else {
...
{
    s_frame_queue = xQueueCreate(1, sizeof(int));
    ...
    if (s_decoded_frame != NULL) {
        ESP_LOGI(TAG,"DECODE_BUFFER=INTERNAL");
    } else {
        s_decoded_frame = heap_caps_malloc(DECODED_BUFFER_BYTES,
                                           MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        ESP_LOGW(TAG,"DECODE_BUFFER=PSRAM");
    }
    if (s_decoded_frame == NULL) {
        return ESP_ERR_NO_MEM;
    }
    ...
    if (xTaskCreatePinnedToCore(frame_display_task, "camera_display", 12288,
...
void app_main(void)
{
    ESP_ERROR_CHECK(digit_runtime_init());
    ESP_LOGI(TAG, "Local MNIST digit recognition, USB camera + TFT");
