...
以下为当前接线。交换 B/C 电机插头并完成方向测试后，物理左轮接在 B 通道；
C 通道由程序保持停止。

2026-08-26 实测：A 为右轮，B 为左轮，两轮前进方向和 GPIO16/17、GPIO8/18
编码器计数均已验证。

| 电机 | PWM | IN1 | IN2 | 编码器 A | 编码器 B |
|---|---:|---:|---:|---:|---:|
| A（当前右轮） | GPIO6 | GPIO15 | GPIO7 | GPIO16 | GPIO17 |
| B（当前左轮） | GPIO11 | GPIO9 | GPIO10 | GPIO8 | GPIO18 |
| C（程序保持停止） | GPIO40 | GPIO42 | GPIO41 | — | — |
| D（停用） | — | — | — | — | — |

| 其他信号 | ESP32-S3 | 说明 |
|---|---:|---|
| STBY | GPIO5 | 四路共用的驱动总使能 |
| ADC | GPIO12 / ADC2_CH1 | 若仍接线则为驱动板电压检测；当前程序不读取 |
...
GPIO4、GPIO39 当前未被固件使用，可留作后续外设；GPIO1/2 也未被当前固件使用，
但重新利用前应确认旧编码器线已经拔除。GPIO40-42 用于保持 C 通道停止，不能复用。
GPIO19/20 用于 USB Serial/JTAG。最终扩展前，应根据实际 ESP32-S3-DevKitC-1
排针丝印逐根核对。
...
- 推荐并已验证的 ESP-IDF：5.4.4；`car.ps1` 也可自动识别其他已安装版本
- 芯片目标：`esp32s3`
- Flash：32 MB
